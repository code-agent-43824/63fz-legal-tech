      ...
      <section className="mx-auto grid w-full max-w-6xl gap-6 px-5 py-8 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="rounded-md border border-slate-200 bg-white p-4 lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-auto">
          ...
          <nav className="mt-4 space-y-1">
            {readerData.toc.map((fragment) => (
              <a
                className={tocLinkClass(fragment.type)}
                href={`#${fragment.id}`}
                key={fragment.id}
              >
                {fragment.title}
...
function tocLinkClass(type: string) {
  const base = "block rounded-md px-3 py-2 text-sm text-slate-700 hover:bg-slate-100";
  if (type === "part") {
    return `${base} ml-3 py-1.5 text-xs`;
  }
  return base;
}
